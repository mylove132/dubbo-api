...
public class ClassUtils {
	...
	public static void parseParameter(List<String> paramterTypeList,
			List<Object> parameterValuesList, RequestTypeArgments arg) {
		try {
			...
			if ("int".equals(className)) {
				...
				paramterTypeList.add(arg.getParamType());
				parameterValuesList.add(isBlank(arg.getParamValue()) ? Constants.INT_ARRAY_DEFAULT : JsonUtils.formJson(arg.getParamValue(), new Type(){}));
				...
				if (className.endsWith("[]")) {
					...
					try {
						...
						if (!isBlank(arg.getParamValue())) {
							//使用通用map格式反序列化值
							Map<String, Object> obj1 = JsonUtils.formJson(arg.getParamValue(), Map.class);
							parameterValuesList.add(obj1);
							if (obj1 == null) {
								//枚举类型的类走字符串序列化
